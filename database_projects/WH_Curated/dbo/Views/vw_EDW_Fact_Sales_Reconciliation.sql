...
CREATE   view [dbo].[vw_EDW_Fact_Sales_Reconciliation] as  
SELECT [RecordID]
	,[CMPNY]
	,[SalesLine_Status]
	,[DATE]
...
-------------------------------------------------------------------------------------------
--  Section 0B
-- Legacy Fact Sales table (no BVBA, TEDA, Open Results , no Budget '24 , '25
-------------------------------------------------------------------------------------------
...
WHERE [Customer No] NOT LIKE 'SHAM%'
	AND Cast([Invoice Date] AS DATE) >= '2022-01-01'       -- Corby update 2-27-26 '01/01/2023'
...
SELECT  ABS(CAST(CAST(
	...
	, null as [Quantity]
	, null as [Quantity_UoM]
	,CONVERT(decimal(38,6), [Net Weight LBS]) as [Quantity_LBs]  --* 2.20462
	,null AS [Volume]
	,null as [Volume_UoM]
	,null as [Price]
	,null as [Currency]
...
WHERE [Customer No] NOT LIKE 'SHAM%' and
[Customer No] not in ('68600F','68700F','69600F','N06044','N08032','N09900','C1E201')
and isNull([Invoice No],'0')<>'0'
and CAST([Invoice Date] as date)>= '2022-01-01'
...
-------------------------------------------------------------------------------------------
	...
	,-1 as [SalesLine_RequestedShipDateKey]
	,'01/01/1900' as [SalesLine_ShipDate]
	,-1 as [SalesLine_ShipDateKey]
	,'01/01/1900' as [SalesLine_DeliveryDate]
	,-1 as [SalesLine_DeliveryDateKey]
