CREATE   PROCEDURE [dbo].[sp_Execute_SCD_Logic_dimProductAll]
AS
BEGIN
	-- Drop intermediate objects if they exist
	DROP TABLE IF EXISTS stage_tbl_DIM_Product_All_New;
	DROP TABLE IF EXISTS stage_tbl_DIM_Product_All_Expired;
	DROP TABLE IF EXISTS stage_tbl_DIM_Product_All_Deleted;
	DROP TABLE IF EXISTS stage_tbl_DIM_Product_All_Final;
	DROP TABLE IF EXISTS stage_tbl_DIM_Product_All_Type1_UpdatesNeeded;
	DROP TABLE IF EXISTS stage_tbl_DIM_Product_All_Type1_OnlyUpdatedRecords;
	DROP TABLE IF EXISTS stage_tbl_DIM_Product_All_Append;
	...
	CREATE TABLE stage_tbl_DIM_Product_All_New AS
	SELECT *
	FROM vw_stage_NewProductsAll;

	-- Step 2: Identify records that need to be expired
	-- Create a table to store records that have changed and need to be expired
	CREATE TABLE stage_tbl_DIM_Product_All_Expired AS
	SELECT Target.*
	FROM tbl_DIM_Product_All AS Target
	JOIN vw_stage_DIM_Product_All_incoming AS Source
	...
	CREATE TABLE stage_tbl_DIM_Product_All_Type1_UpdatesNeeded AS
	SELECT Target.*
	FROM tbl_DIM_Product_All AS Target
	JOIN vw_stage_DIM_Product_All_incoming AS Source
		ON Target.Product_ID = Source.Product_ID
	...
	WHERE Target.RecordStatus = 1
		...
		AND NOT EXISTS (
			SELECT 1
			FROM stage_tbl_DIM_Product_All_Expired AS Expired
	...
	CREATE TABLE stage_tbl_DIM_Product_All_Type1_OnlyUpdatedRecords AS
	SELECT Target.ProductKey
		...
		,Target.RecordStatus
		,Target.Source
	FROM stage_tbl_DIM_Product_All_Type1_UpdatesNeeded AS Target
	JOIN vw_stage_DIM_Product_All_incoming AS Source
		ON Target.Product_ID = Source.Product_ID
			AND Target.CMPNY = Source.CMPNY;
	...
	CREATE TABLE stage_tbl_DIM_Product_All_Deleted AS
	SELECT *
	FROM tbl_DIM_Product_All AS Target
	WHERE Target.RecordStatus = 1
		AND NOT EXISTS (
			SELECT 1
			FROM vw_stage_DIM_Product_All_incoming AS Source
	...
	CREATE TABLE stage_tbl_DIM_Product_All_Final AS
		--Add records from DIM that had no changes
	SELECT *
	FROM tbl_DIM_Product_All
	WHERE RecordStatus = 1
		AND (Product_ID + '~=~' + CMPNY) NOT IN 
			(
			SELECT Product_ID + '~=~' + CMPNY
			FROM stage_tbl_DIM_Product_All_Expired
			UNION
			SELECT Product_ID + '~=~' + CMPNY
			FROM stage_tbl_DIM_Product_All_Deleted
			UNION
			SELECT Product_ID + '~=~' + CMPNY
			FROM stage_tbl_DIM_Product_All_Type1_UpdatesNeeded
			)
	...
	SELECT s.[ProductKey]
		...
		,1 AS RecordStatus
		,s.[Source]
	FROM vw_stage_DIM_Product_All_incoming s
	JOIN stage_tbl_DIM_Product_All_Expired e
	...
	-- Insert new records
	SELECT *
	FROM stage_tbl_DIM_Product_All_New
	
	UNION ALL
	
	--Insert Type 1 updated records
	SELECT *
	FROM stage_tbl_DIM_Product_All_Type1_OnlyUpdatedRecords
	...
	CREATE TABLE stage_tbl_DIM_Product_All_Append AS
	SELECT *
	FROM tbl_DIM_Product_All f
	WHERE NOT EXISTS (
			SELECT 1
			FROM stage_tbl_DIM_Product_All_Final AS d
	...
	SELECT *
	FROM stage_tbl_DIM_Product_All_Final AS f
	ORDER BY Product_ID
		,CMPNY
		,recordeffectivestartdate
	...
	DROP TABLE IF EXISTS tbl_DIM_Product_All;

	-- Recreate the dimension table with the updated records from the append table
	CREATE TABLE tbl_DIM_Product_All AS
	SELECT *
	FROM stage_tbl_DIM_Product_All_Append;
